...
#define EnablePreload_Mosfet_HighLeft    (TIM1->CR1 |= TIM_CR1_ARPE)
#define DisablePreload_Mosfet_HighLeft    (TIM1->CR1 &= ~TIM_CR1_ARPE)
#define EnablePreload_Mosfet_LowLeft    (TIM3->CCMR1 |= TIM_CCMR1_OC1PE)
#define DisablePreload_Mosfet_LowLeft    (TIM3->CCMR1 &= ~TIM_CCMR1_OC1PE)

//--- Exported wrapped functions ---//
#define UpdateTIMSync(X)    do { \
    TIM3->CCR1 = (X); \
    TIM1->ARR = DUTY_50_PERCENT - 1 + (X); \
    } while(0)
